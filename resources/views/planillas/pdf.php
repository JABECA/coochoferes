	...
	<div style="text-align: center;">
 	...
 	<table id='tbldatosresponsables' border='1' align="center" style="width:100%; font-size: 10px;">
 		...
 		<tbody>
 			@for ($i =0; $i <= 30; $i++)
 			<?php 
 				$conductor = isset($planilla[$i]['conductor']) ? $planilla[$i]['conductor'] : false;
 				$supervisor = isset($planilla[$i]['usr_supervisa']) ? $planilla[$i]['usr_supervisa'] : false;
 				$obs_supervisor = isset($planilla[$i]['obs_supervisor']) ? $planilla[$i]['obs_supervisor'] : false;
 				$obs_conductor = isset($planilla[$i]['observaciones']) ? $planilla[$i]['observaciones'] : false;
 				if ($obs_supervisor || $obs_conductor) {
 					$obs = 'C:'.$obs_conductor.';  D:'.$obs_supervisor;
 				}else{
 					$obs = '';
 				}
 			?>
			<tr>
				<td style="width:3%; height:2.4%; ">{{$i+1}}</td>
				<td style="width:20%; height:2.4%;">{{$conductor}}</td>
				<td style="width:20%; height:2.4%;">{{$supervisor}}</td>
				<td style="width:20%; height:2.4%;">{{$obs}}</td>
			</tr>	 		
 	...
 	<script type="text/javascript">
 		...
 		for (var i = 0; i < datos.length; i++) {
		    ...
		    var enlaceEditar = '<i class="fas fa-edit">'
